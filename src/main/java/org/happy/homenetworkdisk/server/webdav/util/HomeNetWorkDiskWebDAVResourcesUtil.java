package org.happy.homenetworkdisk.server.webdav.util;
...
/**
 * <h2>HomeNetWorkDisk虚拟文件系统资源操作工具</h2>
 * <p>
 * 该工具用于实现基于逻辑路径的各类操作。例如根据逻辑路径获取文件、文件夹以及封装为HomeNetWorkDiskResource的资源对象等。 具体功能详见其中各个方法的注释。
 ...
 */
@Component
public class HomeNetWorkDiskWebDAVResourcesUtil {
     ...
     * @return Folder逻辑路径对应的文件夹对象。如果文件夹不存在或逻辑路径不合法，则返回null
     */
     ...
     * @return Node 逻辑路径对应的文件节点对象。如果文件不存在或逻辑路径不合法，则返回null
     */
     ...
     * <h2>根据逻辑路径获取资源对象</h2>
     * <p>
     * 该方法通过逻辑路径返回一个封装好的资源对象，该资源对象是逻辑路径对应的存储在HomeNetWorkDisk的一个虚拟文件。
     * </p>
     *
     * @param path 逻辑路径，必须以“/”起始。例如“/foo”或“/foo/bar/”均合法。
     * @return HomeNetWorkDiskWebDAVResource
     * 资源对象，封装了HomeNetWorkDisk虚拟文件系统中文件或文件夹。
     * 该方法永远不会返回null，当路径不正确时，将会返回一个不存在的WebDAVResource对象。
     */
    public HomeNetWorkDiskWebDAVResource getResource(String path) {
        if (path != null && !path.endsWith("/")) {
            // 从文件（或文件夹）中获取资源对象，Windows客户端在请求文件夹时并不会以“/”结尾，因此无法判定是否为文件夹
            Node node = getNodeByPath(path);
            if (node != null) {
                // 如果该逻辑路径确实对应一个文件，则返回文件节点的资源对象
                return new HomeNetWorkDiskWebDAVResource(path, node, fbu.getFileFromBlocks(node));
            }
        }
        // 否则尝试获取文件夹的资源对象
        Folder folder = getFolderByPath(path);
        return new HomeNetWorkDiskWebDAVResource(path, folder);
    }
     ...
     * @return Folder 如果创建成功则返回文件夹对象，否则返回null（包括未授权的情况）。
     */
