...
import org.happy.homenetworkdisk.server.pojo.FolderView;
import org.happy.homenetworkdisk.server.pojo.RemainingFolderView;
import org.happy.homenetworkdisk.server.pojo.SearchView;
import org.happy.homenetworkdisk.server.service.FolderViewService;
import org.happy.homenetworkdisk.server.util.ConfigureReader;
import org.happy.homenetworkdisk.server.util.FolderUtil;
import org.happy.homenetworkdisk.server.util.HomeNetWorkDiskFFMPEGLocator;
    ...
    private void searchFilesAndFolders(String fid, String key, String account, List<Node> ns, List<Folder> fs) {
        for (Folder f : this.fm.queryByParentId(fid)) {
            if (ConfigureReader.instance().accessFolder(f, account)) {
                ...
                searchFilesAndFolders(f.getFolderId(), key, account, ns, fs);
            }
        }
