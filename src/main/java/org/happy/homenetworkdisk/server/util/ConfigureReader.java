...
/**
 * 
 * <h2>HomeNetWorkDisk配置解析器</h2>
 * <p>
 * 该工具负责读取并解析配置文件，并将结果随时提供给HomeNetWorkDisk服务器业务逻辑以完成相应功能（例如用户认证、权限判定、配置启动端口等）。
 * </p>
 *
 * @version 1.0
 */
public class ConfigureReader {
	...
	private HomeNetWorkDiskProperties serverp;// 配置设置
	private HomeNetWorkDiskProperties accountp;// 账户设置
	...
	private ConfigureReader() {
		...
		this.serverp = new HomeNetWorkDiskProperties();
		this.accountp = new HomeNetWorkDiskProperties();
	...
	private int testServerPropertiesAndEffect() {
		...
		if ("true".equals(serverp.getProperty("mysql.enable"))) {
			...
			dbURL = "jdbc:h2:file:" + fileNodePath + "kift;FILE_LOCK=SOCKET";
			dbUser = "root";
			dbPwd = "301537gY";
		}
		// https支持检查及生效处理
			...
			if ("true".equals(enableHttps)) {
				...
				if (keyFile.isFile()) {
					...
					if (keyFile.isFile()) {
						...
						Printer.instance.print(
								"错误：无法启用https支持，因为HomeNetWorkDisk未能找到https证书文件。您必须在应用主目录内放置PKCS12（必须命名为https.p12）或JKS（必须命名为https.jks）证书。");
	...
	public void createDefaultServerPropertiesFile() {
		...
		try {
			dsp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
					"<This is the default HomeNetWorkDisk server setting file. >");
	...
	private void createDefaultAccountPropertiesFile() {
		...
		try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
			dap.store(accountSettingOut, "<This is the default HomeNetWorkDisk account setting file. >");
		...
		if (accountPropertiesUpdateDaemonThread == null) {
			...
			accountPropertiesUpdateDaemonThread = new Thread(() -> {
				try {
					...
					Printer.instance.print("错误：用户配置文件更改监听失败，该功能已失效，HomeNetWorkDisk无法实时更新用户配置（可尝试重启程序以恢复该功能）。");
				}
			});
