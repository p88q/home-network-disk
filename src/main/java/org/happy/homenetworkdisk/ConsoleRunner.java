...
import org.happy.homenetworkdisk.server.exception.FilesTotalOutOfLimitException;
...
import org.happy.homenetworkdisk.server.util.FileNodeUtil;
import org.happy.homenetworkdisk.util.file_system_manager.FileSystemManager;
import org.happy.homenetworkdisk.util.file_system_manager.pojo.Folder;
import org.happy.homenetworkdisk.util.file_system_manager.pojo.FolderView;
 ...
 * <h2>命令模式启动器</h2>
 * <p>
 * 该启动器将以命令模式启动HomeNetWorkDisk，请执行静态build()方法开启界面并初始化HomeNetWorkDisk服务器引擎。
 * </p>
 * 
 * @version 1.0
 */
public class ConsoleRunner {
	...
	private static HomeNetWorkDiskCtl ctl;
	...
	private ConsoleRunner() {
		Printer.init(false);
		ConsoleRunner.ctl = new HomeNetWorkDiskCtl();
		worker = Executors.newSingleThreadExecutor();
		ConsoleRunner.commandTips = "HomeNetWorkDisk:您可以输入以下指令以控制服务器：\r\n-start 启动服务器\r\n-stop 停止服务器\r\n-exit 停止服务器并退出应用\r\n-restart 重启服务器\r\n-files 文件管理\r\n-status 查看服务器状态\r\n-help 显示帮助文本";
		ConsoleRunner.fsCommandTips = "HomeNetWorkDisk files:您可以输入以下指令进行文件管理：\r\nls 显示当前文件夹内容\r\ncd {“文件夹名称” 或 “--文件夹序号”} 进入指定文件夹（示例：“cd foo” 或 “cd --1”，如需返回上一级请输入“cd ../”）\r\nimport {要导入的本地文件（必须使用完整路径）} 将本地文件或文件夹导入至此\r\nexport {“目标名称” 或 “--目标序号”（省略该项则导出当前文件夹的全部内容）} {要导出至本地的路径（必须使用完整路径）} 将指定文件或文件夹导出本地\r\nrm {“文件夹名称” 或 “--文件夹序号”} 删除指定文件或文件夹\r\nexit 退出文件管理并返回HomeNetWorkDisk控制台\r\nhelp 显示帮助文本";
	}

	/**
	 * 
	 * <h2>以命令模式运行HomeNetWorkDisk</h2>
	 * <p>
	...
	// 执行相应的指令并进行后续处理，该方法为整个HomeNetWorkDisk命令模式的起点。
	private void execute(final String[] args) {
		if (args.length > 0) {
			...
			case "-console": {
				this.startHomeNetWorkDiskByConsole();
				break;
			}
			case "-start": {
			...
			default: {
				Printer.instance.print("HomeNetWorkDisk:无效的指令，使用控制台模式启动请输入参数 -console，直接启动服务器引擎请输入参数 -start，使用UI模式启动请不要传入任何参数。");
				break;
			}
			}
		}
	...
	private void startHomeNetWorkDiskByConsole() {
		Printer.instance.print(" 家庭网盘系统-HomeNetWorkDisk 控制台模式[Console model]");
	...
	private void startServer() {
		...
		if (ConsoleRunner.ctl.started()) {
			...
			Printer.instance.print("HomeNetWorkDisk服务器已启动，可以正常访问了，您可以使用 -status 指令查看运行状态。");
		} else {
			if (ConfigureReader.instance().getPropertiesStatus() != 0) {
				switch (ConfigureReader.instance().getPropertiesStatus()) {
				case ConfigureReader.INVALID_PORT:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，端口设置无效。");
					break;
				case ConfigureReader.INVALID_BUFFER_SIZE:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，缓存设置无效。");
					break;
				case ConfigureReader.INVALID_FILE_SYSTEM_PATH:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，文件系统路径或某一扩展存储区设置无效。");
					break;
				case ConfigureReader.INVALID_LOG:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，日志设置无效。");
					break;
				case ConfigureReader.INVALID_VC:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，登录验证码设置无效。");
					break;
				default:
					Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，请重试或检查设置。");
					break;
				}
			} else {
				Printer.instance.print("错误：HomeNetWorkDisk服务器未能启动，请重试或检查设置。");
	...
	private void exit() {
		Printer.instance.print("执行命令：停止服务器并退出HomeNetWorkDisk...");
	...
	private void awaiting() {
		Thread t = new Thread(() -> {
			...
			try {
				while (true) {
					Printer.instance.print("HomeNetWorkDisk: console$ ");
				...
				Printer.instance.print("错误：读取命令时出现意外导致程序退出，请重启HomeNetWorkDisk。");
			}
		});
		t.start();
	}
	...
	private void fileSystemManagerModel() {
		...
		try {
			...
			Printer.instance.print("错误：无法打开文件系统，该文件系统可能正在被另一个HomeNetWorkDisk占用。");
			return;
		}
		System.out.println("命令帮助：\r\n" + fsCommandTips + "\r\n");
		try {
			while (true) {
				System.out.println("HomeNetWorkDisk: " + currentFolder.getCurrent().getFolderName() + "$ ");
				String command = reader.nextLine();
				// 针对一些带参数指令的操作
		...
		while (true) {
			...
			case "Y":
				try {
					...
					Printer.instance.print("错误：无法读取根目录，请尝试重新打开文件管理系统或重启HomeNetWorkDisk。");
				}
				return;
			case "N":
				return;
